			...
			while ( have_posts() ) :
				...
				if ( ! $is_user_logged_in && $require_login ) :
					...
					if ( $is_user_logged_in && $require_login ) :
						...
						?>
						<p>
							<a class="button" href="<?php echo wp_logout_url( get_permalink() ); ?>" title="<?php printf( esc_attr__( 'Log out of your %s account', 'wpcampus' ), 'WPCampus' ); ?>"><?php _e( 'Logout', 'wpcampus' ); ?></a>
							<span style="margin-left:10px;"><?php printf( __( 'You are logged-in as %1$s%2$s%3$s.', 'wpcampus' ), '<strong>', $current_user->get( 'display_name' ), '</strong>' ); ?></span>
						</p>
						<?php
					endif;
