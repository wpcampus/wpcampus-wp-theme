...
function wpcampus_get_interested_has_location_count() {
	global $wpdb;
	return $wpdb->get_var( "SELECT COUNT(posts.ID) AS count FROM {$wpdb->posts} posts
		INNER JOIN {$wpdb->postmeta} city ON city.post_id = posts.ID AND city.meta_key = 'traveling_city' AND city.meta_value != ''
		INNER JOIN {$wpdb->postmeta} state ON state.post_id = posts.ID AND state.meta_key = 'traveling_state' AND state.meta_value != ''
		WHERE posts.post_type = 'wpcampus_interest' AND posts.post_status = 'publish'" );
}
	...
	if ( $form ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			foreach ( $field['choices'] as $choice ) {
				 ...
				 * Setup field filters.
				 *
				 * Modified to match checkbox filter sample in
				 * get_entries(), per https://www.gravityhelp.com/documentation/article/api-functions/#get_entries
				 */
