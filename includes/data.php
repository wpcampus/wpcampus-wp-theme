...
function wpcampus_get_interested_has_location_count() {
	...
	return $wpdb->get_var( $wpdb->prepare( '%s', $query ) );
}
	...
	if ( $form = GFAPI::get_form( 1 ) ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			foreach ( $field['choices'] as $choice ) {
				 ...
				 * Setup field filters.
				 *
				 * Modified to match checkbox filter
				 * sample in get_entries(),
				 * per https://www.gravityhelp.com/documentation/article/api-functions/#get_entries.
				 */
				$search_criteria['field_filters'][] = array(
					'key'       => "{$field_id}",
					'operator'  => 'in',
